...
from app.workers.tasks_subtitle import enqueue_subtitle_job
...
def process_transcription(job_id: int) -> None:
    ...
    try:
        ...
        segments_en = transcript.segments
        if transcript.language.lower() != "en":
            ...
            segments_en = translator.translate_segments(transcript.segments)

        crud.update_job(db, job, transcript_en=transcript_en)
        enqueue_subtitle_job(job.id, transcript.to_payload(transcript_en, segments_en))
